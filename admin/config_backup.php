  ...
  echo '<table width="100%">'."\n";
        ...
        if ( escapeshellcmd($_POST["action"]) == "download" ) {
          ...
          if (shell_exec('cat /etc/dhcpcd.conf | grep "static ip_address" | grep -v "#"')) {
            $output .= shell_exec("sudo cp -p /etc/dhcpcd.conf $backupDir 2>&1");
          }
          $output .= shell_exec("sudo cp -p /etc/wpa_supplicant/wpa_supplicant.conf $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /etc/NetworkManager/system-connections/*.nmconnection $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /etc/ircddbgateway $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /etc/mmdvmhost $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /etc/dstarrepeater $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /etc/dapnetgateway $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /etc/p25gateway $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /etc/ysfgateway $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /etc/ysf2dmr $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /etc/ysf2nxdn $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /etc/ysf2p25  $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /etc/dgidgateway $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /etc/nxdngateway $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /etc/dmrgateway $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /etc/m17gateway $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /etc/aprsgateway $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /etc/dmr2nxdn $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /etc/dmr2ysf  $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /etc/mobilegps $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /etc/starnetserver $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /etc/timeserver $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /etc/dstar-radio.* $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /etc/pistar-remote $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /etc/hosts $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /etc/hostname $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /etc/bmapi.key $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /etc/motd-ext $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /etc/dapnetapi.key $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /etc/pistar-css.ini $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /etc/crontab $backupDir 2>&1");
          $output .= shell_exec("sudo cp -p /usr/local/etc/RSSI.dat $backupDir 2>&1");
