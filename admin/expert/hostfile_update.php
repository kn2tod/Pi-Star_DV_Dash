...
if ($_SERVER["PHP_SELF"] == "/admin/expert/hostfile_update.php") {

  if (!isset($_GET['ajax'])) {
    system('sudo touch /tmp/hostupdate.log > /dev/null 2>&1 &');
    system('sudo echo "" > /tmp/hostupdate.log > /dev/null 2>&1 &');
    system('sudo bash /usr/local/sbin/HostFilesUpdate.sh > /dev/null 2>&1 &');
  }
  ...
  if (!isset($_GET['ajax'])) {
    ...
    if (file_exists('/tmp/hostfileupdate.log')) {
      $_SESSION['update_offset'] = filesize('/tmp/hostfileupdate.log');
    } else {
  ...
  if (isset($_GET['ajax'])) {
    ...
    $handle = fopen('/tmp/hostfileupdate.log', 'rb');
...
?>
  ...
  <head>
    ...
    $(function() {
      $.repeat(1000, function() {
        $.get('/admin/expert/hostfile_update.php?ajax', function(data) {
  ...
  <table width="100%">
  <tr><th>Host File Update</th></tr>
  <tr><td align="left"><div id="tail">Starting host files update, please wait...<br /></div></td></tr>
  </table>
  </div>
  <div class="footer">
